...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await getAllProductos(); // Llama a getAllProductos para obtener los productos al montar
      await getAllVentas(); // Llama a getAllVentas para obtener las ventas al montar
    };
    fetchData();
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de eliminar este producto?')) {
      ...
      await getAllProductos(); // Vuelve a obtener los productos después de eliminar uno
    }
  };
